...
public interface AstaRepository extends CrudRepository<Asta, Integer>{
	...
	@Query("SELECT a FROM Asta a WHERE a.id_creatore = :idCreatore ORDER BY a.nome")
	List<Asta> filtraPerUtente(@Param("id_creatore") int idCreatore);

	@Query("SELECT a FROM Asta a JOIN Offerta o ON a.id = o.id_asta WHERE o.id_utente = :idUtente ORDER BY a.nome")
	List<Asta> filtraPerOfferteUtente(@Param("id_utente") int idUtente);
